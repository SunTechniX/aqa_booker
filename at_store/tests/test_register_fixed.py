...
# =============================================================================
# БОНУС: Тест логина с готовым аккаунтом (если регистрация не нужна)
# =============================================================================

def test_api_login_only(page: Page):
    """
    Логин через API с ЗАРАНЕЕ СОЗДАННЫМ аккаунтом.
    
    ИНСТРУКЦИЯ:
    1. Зарегистрируйся вручную на сайте
    2. Убедись, что можешь войти
    3. Вставь свои loginname и пароль ниже
    """
    # 🔥 🔥 🔥 ЗАМЕНИ НА СВОИ 🔥 🔥 🔥
    U_LOGIN = "твой_логин_без_собаки"      # ← Только буквы и цифры!
    U_PASS = "твой_пароль"
    
    page.goto("https://www.automationteststore.com/index.php?rt=account/login")
    page.wait_for_load_state("networkidle")
    
    csrf = page.locator("#loginFrm [name='csrftoken']").get_attribute("value")
    csrf_inst = page.locator("#loginFrm [name='csrfinstance']").get_attribute("value")
    
    payload = {
        "csrftoken": csrf,
        "csrfinstance": csrf_inst,
        "loginname": U_LOGIN,    # 🔑 loginname, а не email!
        "password": U_PASS,
        "account": "login"
    }
    ...
    assert any(c['name'] == 'customer' for c in cookies), "Нет куки 'customer'"
    
    print(f"✅ Логин успешен: {U_LOGIN}")